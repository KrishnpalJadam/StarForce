...
const handleView = (req) => {
  ...
  const renderPagination = () => {
    ...
    return pages.map((page, index) => (
      <button
        key={index}
        className={`px-3 py-1 rounded ${page === currentPage ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-800'
          } mx-1`}
  ...
  return (
    ...
    `}
                      value={req.status}
                      onChange={(e) => console.log("Selected:", e.target.value)} // Only UI, no functionality
                    >
